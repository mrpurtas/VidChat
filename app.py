...
with tab_url:
    ...
    if submit_btn:
        ...
        with st.spinner("AŞAMA-2: Sorunuz yanıtlanıyor..."):
            AI_Response, relevant_documents = raghelper.rag_with_video_transricpt(transcript_docs=st.session_state.current_transcript_docs, prompt=prompt)
        st.info("YANIT:")
...
with tab_search:
    ...
    with col_left:
        ...
        video_count = st.slider(label="Sonuc Sayısı", min_value=1, max_value=5, value=5, key="search_video_count")
        ...
        if selected_video:
            ...
            if search_ask_btn:
                if st.session_state.current_transcript_docs != selected_video:
                    with st.spinner("AŞAMA-1: Video metni hazırlanıyor..."):
                ...
                with st.spinner("AŞAMA-2: Sorunuz yanıtlanıyor..."):
                    AI_Response, relevant_documents = raghelper.rag_with_video_transricpt(transcript_docs=st.session_state.current_transcript_docs, prompt=search_prompt)
                st.info("YANIT:")
                st.markdown(AI_Response)
                st.divider()
